...
async def subscription(message: Message):
    ...
    try:
        subscription = Subscription.objects.get(telegram_id=message.from_user.id)
    ...
    remaining_time = subscription.expires_at - timezone.now()
...
async def handle_payment_button(callback_query: CallbackQuery):
    ...
    except BotMessageForSubscription.DoesNotExist:
        await callback_query.answer("Платежное сообщение не настроено.")
        return

    # Отправляем платежное сообщение
    await callback_query.message.answer(bot_message.text)
