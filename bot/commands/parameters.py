...
async def set_profit(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Прибыль успешно изменена на: {profit}")
    except ValueError:
...
async def set_pause(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Пауза успешно изменена на: {pause}")
    except ValueError:
        await message.answer("❌ Некорректное значение. Попробуйте снова.")
    finally:
        await state.clear()
...
async def set_loss(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Падение успешно изменено на: {loss}")
    except ValueError:
        await message.answer("❌ Некорректное значение. Попробуйте снова.")
    finally:
        await state.clear()
...
def save_user_parameter(user_id: int, param: str, value: float):
    try:
        ...
        user.save()
        logger.info(f"✅ Saved {param}={value} for user {user_id}")
    except User.DoesNotExist:
